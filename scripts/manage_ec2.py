...
def get_instance(ec2):
    ...
    """
    print(
        f"Searching for instance with Tag:Project={PROJECT_TAG} and Name=*{TAG_NAME_SUFFIX} in {REGION}..."
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Manage the MLflow/Airflow EC2 instance."
    )
    parser.add_argument(
        "action",
        choices=["start", "stop", "status"],
        help="Action to perform: start, stop (pause), or check status.",
    )
