    ...
    for exp in experiments:
        ...
        try:
            ...
            versions = client.search_model_versions(
                f"name='{reg_model_name}' and run_id='{run_id}'"
            )
            if versions:
                ...
                result = mlflow.register_model(f"runs:/{run_id}/model", reg_model_name)
                target_version = client.get_model_version(
                    reg_model_name, result.version
                )

            # 4. Assign @HMG alias
            print(f"  Assigning alias @HMG to version {target_version.version}...")
            client.set_registered_model_alias(
                name=reg_model_name, alias="HMG", version=target_version.version
            )
