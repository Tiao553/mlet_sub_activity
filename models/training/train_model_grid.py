...
os.environ[
    "TF_XLA_FLAGS"
] = "--tf_xla_enable_xla_devices=false --tf_xla_auto_jit=0 --tf_xla_cpu_global_jit=0"
...
def parse_args():
    ...
    parser.add_argument(
        "--sequence_length", type=int, default=24, help="Input sequence length"
    )
    parser.add_argument(
        "--feature_set",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="lstm",
        choices=["lstm", "gru", "bi_lstm"],
        help="Model architecture",
    )
    parser.add_argument(
        "--hidden_units_1", type=int, default=128, help="Units in first layer"
    )
    parser.add_argument(
        "--hidden_units_2", type=int, default=64, help="Units in second layer"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Number of stacked layers (if processed)",
    )
    ...
    # MLflow
    parser.add_argument(
        "--mlflow_uri", type=str, required=True, help="MLflow Tracking URI"
    )
    parser.add_argument(
        "--experiment_name",
        type=str,
...
def build_tf_model(input_shape, args):
    ...
    if args.model_type == "bi_lstm":
        model.add(
            tf.keras.layers.Bidirectional(
                tf.keras.layers.LSTM(args.hidden_units_1, return_sequences=True)
            )
        )
        model.add(tf.keras.layers.Dropout(args.dropout))
        model.add(
            tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(args.hidden_units_2))
        )
    ...
    opt = (
        ...
        else tf.keras.optimizers.SGD(learning_rate=args.learning_rate)
    )
...
class PyTorchModel(torch.nn.Module):
    def __init__(self, input_size, args):
        ...
        fc_input_dim = (
            args.hidden_units_1 * 2
            if args.model_type == "bi_lstm"
            else args.hidden_units_1
        )
...
def main():
    ...
    run_name = (
        f"{args.symbol}_{args.period}_{args.interval}_{args.framework}_{timestamp}"
    )
    ...
    with mlflow.start_run(run_name=run_name) as run:
        ...
        try:
            df = yf.download(
                args.symbol, period=args.period, interval=args.interval, progress=False
            )
        ...
        if len(df) < 35:
            ...
            print(
                f"WARNING: {msg}. Skipping {args.symbol} {args.period}/{args.interval}."
            )
        ...
        )  # Need at least seq_len + some data for split
        ...
        if np.isnan(dataset).any() or np.isinf(dataset).any():
            print(
                "WARNING: Dataset contains NaNs or Infs after pre-processing. filling with 0"
            )
        ...
        if len(X) == 0:
            print(
                f"WARNING: No sequences created for {args.symbol} (len={len(df)}, seq_len={args.sequence_length}). Skipping."
            )
        ...
        if split == 0 or split == len(X):
            print(
                f"WARNING: Insufficient samples for split ({len(X)} samples). Skipping."
            )
        ...
        if args.framework == "tensorflow":
                ...
                try:
                    ...
                    try:
                        mlflow.register_model(
                            model_uri=model_info.model_uri, name=reg_model_name
                        )
            ...
            criterion = (
                torch.nn.MSELoss() if args.loss_function == "mse" else torch.nn.L1Loss()
            )
            optimizer = (
                torch.optim.Adam(model.parameters(), lr=args.learning_rate)
                if args.optimizer == "adam"
                else torch.optim.SGD(model.parameters(), lr=args.learning_rate)
            )
        ...
        plot_path = f"prediction_{uuid.uuid4().hex}.png"
