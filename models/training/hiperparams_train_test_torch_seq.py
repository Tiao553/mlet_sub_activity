...
class TimeSeriesLSTM(nn.Module):
    def __init__(
        self, input_size, hidden_size_1, hidden_size_2, dropout_rate, output_size
    ):
...
def train_and_evaluate(X_train, y_train, X_test, y_test, params, n_features):
    ...
    y_train_tensor = torch.tensor(y_train, dtype=torch.float32).to(
        device
...
def main():
    ...
    for trial in range(n_trials):
        ...
        try:
            rmse, mae, _, model, y_inv, p_inv = train_and_evaluate(
                X_train, y_train, X_test, y_test, params, n_features=len(features)
            )
            print(
                f"RMSE (Mean over {FORECAST_HORIZON} steps): {rmse:.4f} - MAE: {mae:.4f}"
            )
    ...
    if best_model:
        ...
        plt.title(
            f"Previsão Seq2Seq (Step 1 of {FORECAST_HORIZON}) - {symbol} (Melhor Modelo PyTorch)"
        )
