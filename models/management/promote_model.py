...
class ModelPromoter:
    ...
    def transition_and_tag(self, model_name, version, stage, alias, tags=None):
        ...
        self.client.transition_model_version_stage(
            ...
            archive_existing_versions=(
                stage == "Production"
            ),  # Only archive others if Production
        )
    ...
    def evaluate_and_promote(self, exp):
        ...
        logger.info(
            f"Best Candidate: Run {candidate_run_id} | {self.metric}: {candidate_metric:.4f}"
        )
        ...
        candidate_version = next(
            (v for v in all_versions if v.run_id == candidate_run_id), None
        )

        if not candidate_version:
            logger.warning(
                f"Run {candidate_run_id} is not registered as a version of {reg_model_name}. Skipping."
            )
            return

        # 4. Get Current Champion (Production) and Challenger (Staging)
        champion_version = self.get_latest_version_by_stage(
            reg_model_name, "Production"
        )
        ...
        if not champion_version:
            logger.info(
                f"No Champion found for {reg_model_name}. Promoting Candidate to Production."
            )
            self.transition_and_tag(
                ...
                "champion",
                tags={
                    "promotion_reason": "init_champion",
        ...
        try:
            ...
            logger.warning(
                "Could not fetch Champion metrics. Assuming Candidate is better."
            )
            champion_metric = float("inf")

        logger.info(
            f"Current Champion: v{champion_version.version} | {self.metric}: {champion_metric:.4f}"
        )
        ...
        logger.info(
            f"Improvement: {improvement_pct:.2%} (Threshold: {self.threshold:.2%})"
        )
        ...
        if improvement_pct > self.threshold:
            ...
            logger.info(
                f"Demoting Old Champion v{champion_version.version} to Staging (@challenger)"
            )
            self.transition_and_tag(
                reg_model_name,
                ...
                "challenger",
                tags={
                    "demotion_reason": "defeated_by_candidate",
                    "demotion_date": datetime.datetime.now().isoformat(),
                },
            )
            ...
            if not challenger_version:
                ...
                try:
                    ...
                    if challenger_metric and candidate_metric < challenger_metric:
                        promote_to_challenger = True
                        logger.info(
                            f"Candidate ({candidate_metric:.4f}) is better than current Challenger ({challenger_metric:.4f})"
                        )
                except:
            ...
            if promote_to_challenger:
                if candidate_version.version == champion_version.version:
                    ...
                    self.transition_and_tag(
                        ...
                        "challenger",
                        tags={
                            "promotion_reason": "best_alternative",
                            "promotion_date": datetime.datetime.now().isoformat(),
                        },
                    )
            else:
                logger.info(
                    "Candidate is not better than current Challenger. No action."
                )
