...
def approve_model(model_name, version):
    ...
    try:
        ...
        if "HMG" not in mv.aliases:
            print(
                "WARNING: This model version is NOT tagged as @HMG. Proceeding anyway as requested by operator."
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Approve a model candidate to Champion."
    )
    parser.add_argument(
        "--model", type=str, required=True, help="Registered Model Name"
    )
    parser.add_argument(
        "--version", type=str, required=True, help="Model Version to approve"
    )
